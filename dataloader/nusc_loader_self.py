...
class NuScenesloader:
    def __init__(self, detection_path, first_token_path,final_token_path, token_path, config, suffle = False,token = None,logger = None):
        """
        ...
        self.detector = load_file(detection_path,logger)["results"]
        all_sample_token = list(self.detector.keys())

        self.seq_token = load_file(token_path,logger)

        self.suffle = suffle

        # if suffle:
        #     self.seq_token = random.shuffle(self.seq_token)

        self.all_sample_token = [item for sublist in self.seq_token for item in sublist]

        # print (len(self.all_sample_token))
        # print (len(self.detector.keys()))
        # exit()


        # assert all_sample_token == self.all_sample_token

        self.seq_first_token = load_file(first_token_path,logger)
        self.seq_final_token = load_file(final_token_path,logger)
        ...
        if token is not None:
            self.all_sample_token = token

    def suffle_list(self):
        if self.suffle:
            seq_token = [sublist.copy() for sublist in self.seq_token]
            random.shuffle(seq_token)
        else:
            seq_token = self.seq_token

        # if self.suffle:
        #     seq_token = random.sample(self.seq_token, 100)
        # else:
        #     seq_token = self.seq_token

        self.all_sample_token = [item for sublist in seq_token for item in sublist]        
    ...
    def __getitem__(self, item) -> dict:
        ...
        data_info = {
            ...
            'is_final_frame': curr_token in self.seq_final_token,
            'timestamp': item,
