...
class KalmanFilter:
    ...
    def addFrameObject(self, timestamp: int, tra_info: dict, mode: str = None) -> None:
        ...
        if mode == 'update':
            ...
            # frame_object.update_state.retain_grad()
        elif mode == 'predict':
...
class LinearKalmanFilter(KalmanFilter):
    ...
    """
    def __init__(self, timestamp: int, config: dict, track_id: int, det_infos: dict,model) -> None:
        # init basic infos
...
class ModelKalmanFilter(KalmanFilter):
    def __init__(self, timestamp: int, config: dict, track_id: int, det_infos: dict,model,class_id) -> None:
        ...
        # self.model = globals()[self.model](self.has_velo, self.dt) if self.model in ['CTRA', 'CTRV' , 'BICYCLE'] \
        ...
        self.initialize(det_infos,track_id)
    ...
    def getOutputInfo(self, state: torch.tensor) -> np.array:
        ...
        # return state vector except tra score and tra class
        inner_state = self.train_model.getOutputInfo(state)
        return np.append(inner_state, np.array([-1, self.class_label]))


    def initialize(self, det_infos: dict,track_id: int) -> None:
        self.train_model.init_hidden_KNet(track_id)
        self.state = self.train_model.getInitState(det_infos)
        tra_infos = {
            'inner_state': self.state,
            'exter_state': det_infos['np_array']
        }
        ...
        self.m1x_posterior = self.state.cuda()
        self.m1x_posterior_previous = self.m1x_posterior 
        self.m1x_prior_previous = self.m1x_posterior
        self.y_previous = self.train_model.StateToMeasure(self.state)
        self.y = self.train_model.StateToMeasure(self.state)
        self.m1y = self.train_model.StateToMeasure(self.state)

    def predict(self, timestamp: int) -> None:

        # assert self.state is self[timestamp-1].update_state

        self.state = self.train_model.stateTransition(self.state)
    ...
    def update(self, timestamp: int, det: dict = None) -> None:
        # corner case, no det for updating
        if det is None:
            # assert self.state.shape == (10, 1)
            # assert self.state is self[timestamp].predict_state
            # output_info = self.getOutputInfo(self.state)
            # tra_infos = {
            #     'inner_state': self.state,
            #     'exter_state': output_info
        ...
        self.y = torch.tensor(meas_info).float().to(self.device)
        self.y_previous = self.y_previous.to(self.device)
        self.m1y = self.m1y.to(self.device)
        self.m1x_posterior = self.m1x_posterior.to(self.device)
        self.m1x_posterior_previous = self.m1x_posterior_previous.to(self.device)
        self.m1x_prior_previous = self.m1x_prior_previous.to(self.device)

        obs_diff = self.y - self.y_previous
        obs_innov_diff = self.y - self.m1y 

        # fw_evol_diff = self.m1x_posterior - self.m1x_posterior_previous
        # fw_update_diff = self.m1x_posterior - self.m1x_prior_previous
        fw_evol_diff = self.state.to(self.device) - self.m1x_posterior
        fw_update_diff = self.state.to(self.device) - self.m1x_posterior_previous

        KG = self.train_model(obs_diff,obs_innov_diff,fw_evol_diff,fw_update_diff,track_id=self.tracking_id,class_id = self.class_id).squeeze(0)   

        # Innovation
        dy = obs_innov_diff # [batch_size, n, 1]

        # Compute the 1-st posterior moment
        INOV = torch.matmul(KG, dy)
        self.m1x_posterior_previous = self.m1x_posterior
        self.m1x_posterior = self.state.to(self.device) + INOV

        #self.state_process_posterior_0 = self.state_process_prior_0
        self.m1x_prior_previous = self.state

        # update y_prev
        self.y_previous = self.m1y #y

        #m1x_posterior_previous

        self.state = self.m1x_posterior.cpu() 

        # output updated state to the result file
        self.train_model.warpStateYawToPi(self.state)
        output_info = self.getOutputInfo(self.state)
