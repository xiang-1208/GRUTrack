...
class Tracker:
    def __init__(self, config,model):
        ...
        self.has_velo = config['basic']['has_velo']

        from nuscenes.eval.common.config import config_factory as track_configs
        self.dist_th_tp = track_configs("tracking_nips_2019").dist_th_tp

        self.model = model

        self.train = False

        # # self.optimizer = torch.optim.Adam(self.model.parameters(), lr=5e-7, weight_decay=1e-3)
        # self.optimizer = torch.optim.AdamW(self.model.parameters(), lr=1e-5, weight_decay=1e-5,eps=1e-8)
        
        # # scheduler = optim.lr_scheduler.StepLR(optimizer_model, step_size=10, gamma=0.5)
        # # self.scheduler = torch.optim.lr_scheduler.OneCycleLR(self.optimizer, 1e-7, 20*700,
        # #     pct_start=0.05, cycle_momentum=False, anneal_strategy='linear')
        # # self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, 50, eta_min=1e-6, last_epoch=-1)
        # self.scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(self.optimizer,T_0=50,T_mult=2,eta_min=1e-6)

        # self.optimizer = torch.optim.SGD(self.model.parameters(),lr=1e-5,momentum=0.9,weight_decay=1e-5)
    
        self.optimizer = torch.optim.AdamW(self.model.parameters(), lr=1e-5, weight_decay=1e-3,eps=1e-8)
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(self.optimizer,T_0=50,T_mult=2,eta_min=1e-6)

        # self.optimizer = torch.optim.AdamW(self.model.parameters(), lr=1e-4, weight_decay=1e-5,eps=1e-8)
        # self.scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(self.optimizer,T_0=50,T_mult=2,eta_min=1e-5)

        # self.loss_fn = nn.MSELoss(reduction='mean').cuda()
        self.loss_fn = nn.SmoothL1Loss(reduction='mean').cuda()

        self.MSE_trainbatch_linear_LOSS = [] 
        
        #1e-5 到 1e-6的动态台调整率最佳
    ...
    def gt_association(self,gt_data):
        asso_traj = {}
        for id in self.valid_tras.keys():
            if self.valid_tras[id].have_gt == False:
                asso_traj[id] = self.valid_tras[id][self.frame_id]

        asso_gt = self.asso_gt

        # distance_cost = np.array(np.size(len(asso_gt),len(asso_traj)))
        distance_cost = np.zeros((len(asso_gt),len(asso_traj)))
        for traj_index,traj_id in enumerate(asso_traj):
            for gt_index,gt_id in enumerate(asso_gt):
                traj_center = asso_traj[traj_id].predict_box.center
                traj_label = asso_traj[traj_id].predict_box.name
                gt_label = asso_gt[gt_id]["tracking_name"]
                if (traj_label == gt_label):
                    gt_center = np.array(asso_gt[gt_id]["translation"])
                    distance_xy = np.linalg.norm(traj_center[:2] - gt_center[:2])
                    distance_cost[gt_index,traj_index] = distance_xy
                else:
                    distance_cost[gt_index,traj_index] = 1e6

        from scipy.optimize import linear_sum_assignment

        rids, cids = linear_sum_assignment(distance_cost)

        # 定义一个过滤函数，用于过滤有效的索引
        def is_valid_index(i, j):
            return 0 <= i < len(distance_cost) and 0 <= j < len(distance_cost[0]) and not distance_cost[i, j] >= self.dist_th_tp
        # 使用filter函数过滤rid和col中的索引
        valid_indices = list(filter(lambda idx: is_valid_index(idx[0], idx[1]), zip(rids, cids)))

        if not valid_indices:
            return

        rids, cids = zip(*valid_indices)
        rids, cids = list(rids), list(cids)
        assert (len(rids) == len(cids))

        for i in range(len(rids)):
            # rids[i] = list(asso_gt.keys())[rids[i]]
            cids[i] = list(asso_traj.keys())[cids[i]]

        for index,tra_index in enumerate(cids):
            self.valid_tras[tra_index].have_gt = True
            self.valid_tras[tra_index].gt_id = gt_data[rids[index]]["tracking_id"]

    def tracking(self, data_info: dict,gt_data: list = None,logger = None,loss_back = None,epoch = None) -> None:
        """
        ...
        # if data_info['is_first_frame']: self.gt_association(gt_data)
        ...
        if self.train:
            ...
            have_traj = []
            for id in self.valid_tras.keys():
                if self.valid_tras[id].have_gt is True:
                    have_traj.append(self.valid_tras[id].gt_id)
            for box in gt_data:
                gt_id = box["tracking_id"]
                self.gt[gt_id] = box
                if gt_id not in have_traj:
                    self.asso_gt[gt_id] = box
        ...
        def quaternion_to_euler(q):
            ...
            # 提取旋转矩阵中的欧拉角
            theta_x = torch.atan2(rotation_matrix[2, 1], rotation_matrix[2, 2])
            theta_y = torch.atan2(-rotation_matrix[2, 0], torch.sqrt(rotation_matrix[2, 1]**2 + rotation_matrix[2, 2]**2))
            theta_z = torch.atan2(rotation_matrix[1, 0], rotation_matrix[0, 0])

            # 将弧度转换为角度
            euler_angles = torch.stack([theta_x, theta_y, theta_z])

            return euler_angles

        if self.train:
            for id in self.valid_tras.keys():
                if self.valid_tras[id].have_gt and self.valid_tras[id].gt_id in self.gt:
                                        
                    input_state = self.valid_tras[id][self.frame_id].update_state

                    if input_state is None:
                        continue

                    # input_state.retain_grad()

                    # input_state = torch.tensor(input_state)

                    input = self.model.StateToMeasure(input_state).cuda()

                    gt_id = self.valid_tras[id].gt_id
                    translation = torch.tensor(self.gt[gt_id]["translation"]).unsqueeze(-1) #list
                    rotation = self.gt[gt_id]["rotation"] #list
                    yaw = quaternion_to_euler(rotation)[2].unsqueeze(-1).unsqueeze(-1)
                    size = torch.tensor(self.gt[gt_id]["size"]).unsqueeze(-1)
                    if (self.has_velo):
                        v = torch.tensor(self.gt[gt_id]["velocity"]).unsqueeze(-1)
                        target = torch.cat((translation,size,v,yaw),dim=0).float().cuda()
                    else:
                        target = torch.cat((translation,size,yaw),dim=0).float().cuda()

                    loss = self.loss_fn(input,target).cuda()

                    tracking_ids_list = tracking_ids.tolist()

                    if (id in tracking_ids_list):
                        tracking_id = tracking_ids_list.index(id)
                        det = data_info['box_dets'][tracking_id]
                        re = (target[0].item() - det.center[0])/(target[0].item()+1e-9)
                        with open('/home/wxx/vel/Poly-MOT/output.txt', 'a', encoding='utf-8') as f:
                            f.write(f"{re}\n")  # 自动换行  


                    # re = (input[0] - target[0]).item()
                    # # 追加写入文件
                    # with open('/home/wxx/vel/Poly-MOT/output.txt', 'a', encoding='utf-8') as f:
                    #     f.write(f"{re}\n")  # 自动换行                    
                    ...
                    self.MSE_trainbatch_linear_LOSS.append(loss)
                    
                # else:
                    # target_state = self.valid_tras[id].motion_management_model.y
                    # input_state = self.valid_tras[id].motion_management_model.m1y   

                    # if target_state is not  None:

                    #     # if ((not input_state.isnan()) and (not target_state.isnan())):
                    #     loss = self.loss_fn(input_state,target_state).cuda()
                    #     if not loss.isnan():
                    #         self.num +=1
                    #         self.MSE_trainbatch_linear_LOSS.append(loss) 
                    ...
                    # target_state = self.valid_tras[id].motion_management[self.frame_id].update_state
                    # if target_state is not  None:
        ...
        # view(dict_track_res['box_track_res'],data_info['sample_token'])
        ...
        if self.train:        
            if data_info['is_final_frame']: 
                # self.MSE_trainbatch_linear_LOSS = 0

                # Compute Training Loss
                MSE_trainbatch_linear_LOSS = torch.mean(torch.stack(self.MSE_trainbatch_linear_LOSS))
                logger.info ("%.9f",MSE_trainbatch_linear_LOSS)
                loss_back[0] = MSE_trainbatch_linear_LOSS.detach().clone()
                loss_back[1] = self.optimizer.state_dict()['param_groups'][0]['lr']
                # logger.info ("loss: %.9f",loss_back)

                if MSE_trainbatch_linear_LOSS.isnan():
                    self.MSE_trainbatch_linear_LOSS.clear()
                ...
                logger.info ("LR: %.9f", self.optimizer.state_dict()['param_groups'][0]['lr'])
                self.optimizer.step() 
    ...
    def tras_predict(self) -> None:
        ...
        for tra_id, tra in self.valid_tras.items():
            ...
            pred_bms.append(pred_object.predict_bms)  #框的四个顶点
