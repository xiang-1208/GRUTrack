...
from motion_module.kalman_filter import LinearKalmanFilter, ExtendKalmanFilter ,ModelKalmanFilter


class Trajectory:
    def __init__(self, timestamp: int, config: dict, track_id: int, det_infos: dict,model):
        # init basic infos
        self.timestamp = timestamp
        self.cfg, self.tracking_id, self.class_label = config, track_id, int(det_infos['np_array'][-1])
        ...
        # KF_type = 'ExtendKalmanFilter'
        # self.motion_management = globals()[KF_type](timestamp, config, track_id, det_infos,model = model)
        # if (KF_type == 'ExtendKalmanFilter'):
        self.motion_management = ModelKalmanFilter(timestamp, config, track_id, det_infos,model = model,class_id = self.class_label)
        self.have_gt = False
        self.gt_id = None
    ...
    def state_predict(self, timestamp: int) -> None:
        ...
        # self.motion_management_model.predict(timestamp)
    ...
    def state_update(self, timestamp: int, det: dict = None) -> None:
        ...
        # self.motion_management_model.update(timestamp, det)
